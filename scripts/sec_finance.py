...
def _extract_entries(facts: dict, concept_list: list) -> list:
    """
    Extract entries across all matching concepts (union) from fact dict.
    For each (end, form) pair, keeps the entry with the latest 'filed' date.
    ...
    """
    # First pass: collect best entry per (end, form) per concept per currency
    best: dict = {}  # (end, form, concept) -> entry (by latest filed)
    ...
    for concept in concept_list:
        ...
        for e in units[currency]:
            k = (e.get("end", ""), e.get("form", ""), concept)
            existing = best.get(k)
